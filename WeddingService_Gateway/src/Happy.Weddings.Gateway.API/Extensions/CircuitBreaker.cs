    ...
    public static class CircuitBreaker
    {
                                                            ...
                                                            IConfiguration configuration)
        {
            ...
            services.AddHttpClient(BlogServiceOperation.serviceName, c => { c.BaseAddress = new Uri(servicesConfig.Blog); })
                    .AddTransientHttpErrorPolicy(p => p.CircuitBreakerAsync(10, TimeSpan.FromMinutes(2)));
            services.AddHttpClient(WalletServiceOperation.serviceName, c => { c.BaseAddress = new Uri(servicesConfig.Wallet); })
                    .AddTransientHttpErrorPolicy(p => p.CircuitBreakerAsync(10, TimeSpan.FromMinutes(2)));
            services.AddHttpClient(LeadServiceOperation.serviceName, c => { c.BaseAddress = new Uri(servicesConfig.Lead); })
                    .AddTransientHttpErrorPolicy(p => p.CircuitBreakerAsync(10, TimeSpan.FromMinutes(2)));
