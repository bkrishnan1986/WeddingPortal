...
using Happy.Weddings.Gateway.Core.DTO.Identity.Multicode;
...
namespace Happy.Weddings.Gateway.Service.Services.v1.Identity
{
    ...
    public class MulticodeService : IMulticodeService
    {
        ...
        /// <param name="request">The request.</param>
        /// <returns></returns>
        public async Task<APIResponse> UpdateMultiCode(MulticodeIdDetails details, UpdateMulticodeIdentityRequest request)
        {
            try
            {
                var client = httpClientFactory.CreateClient(IdentityServiceOperation.serviceName);
                ...
                var response = await client.PutAsync(servicesConfig.Wallet + IdentityServiceOperation.UpdateMultiCode(details.Code), contentPost);
                return new APIResponse(response.StatusCode);
            }
            catch (Exception ex)
            {
